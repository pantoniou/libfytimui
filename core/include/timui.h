 ...
 * timui_full_redraw, resume), and a shrink COMPACTS the band upward: the
 * uncovered rows are erased with a targeted erase after the repaint, in
 * the same synchronized frame, so the chrome moves in one atomic hop.
 * timui_inline_commit QUEUES finished
